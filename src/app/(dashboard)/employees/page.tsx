...
function EmployeesContent() {
  ...
  React.useEffect(() => {
    if (currentRole === 'employee') {
      ...
      const selfId = empSelf?.id || empSelf?.employeeCode || currentUser?.employeeId;
      if (selfId) {
  ...
  const filteredEmployees = employees.filter((emp) => {
    if (currentRole === 'employee') {
      ...
      const selfId = empSelf?.id || currentUser?.employeeId;
      const selfCode = empSelf?.employeeCode;
      return (
        (selfId && (emp.id === selfId || emp.employeeCode === selfId)) ||
        (selfCode && emp.employeeCode === selfCode) ||
        (currentUser?.email && emp.email && emp.email.toLowerCase() === currentUser.email.toLowerCase())
      );
    }
    ...
    const matchesStatus =
      selectedStatus === 'all' ||
      emp.employmentStatus?.toLowerCase() === selectedStatus.toLowerCase();
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
        ...
        {can('create', 'employee_records') && (
          <Dialog open={onboardModalOpen} onOpenChange={(open) => { setOnboardModalOpen(open); if (!open) setFormError(''); }}>
            <DialogTrigger asChild>
              <Button className="rounded-xl gap-2 shadow-md bg-indigo-600 hover:bg-indigo-700 text-white w-full sm:w-auto">
                <Plus className="h-4 w-4" />
                <span>Onboard New Employee</span>
              </Button>
            </DialogTrigger>
            <DialogContent className="max-w-3xl w-[95vw] sm:w-full p-4 sm:p-6 max-h-[85vh] sm:max-h-[90vh] overflow-y-auto rounded-2xl sm:rounded-3xl">
              <DialogHeader>
                <DialogTitle className="text-lg font-bold">Onboard New Employee Profile</DialogTitle>
              </DialogHeader>
      ...
      {currentRole !== 'employee' && (
        <Card className="p-4 sm:p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 sm:gap-4 w-full items-center">
            <div className="relative sm:col-span-2 lg:col-span-5">
              ...
              <input
                ...
                className="w-full h-11 pl-10 pr-4 rounded-xl text-xs sm:text-sm bg-slate-50/80 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-900 transition-all shadow-xs"
              />
            </div>
            ...
            <div className="sm:col-span-1 lg:col-span-3">
              <select
                ...
                className="w-full h-11 px-3.5 rounded-xl text-xs sm:text-sm bg-slate-50/80 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-900 cursor-pointer transition-all shadow-xs"
              >
            ...
            <div className="sm:col-span-1 lg:col-span-2">
              <select
                ...
                className="w-full h-11 px-3.5 rounded-xl text-xs sm:text-sm bg-slate-50/80 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-900 cursor-pointer transition-all shadow-xs capitalize"
              >
            ...
            <div className="sm:col-span-2 lg:col-span-2 flex items-center justify-between sm:justify-center gap-2 bg-slate-50 dark:bg-slate-800/70 px-3.5 rounded-xl border border-slate-200 dark:border-slate-700/60 h-11 text-xs text-slate-500">
              <span className="font-semibold text-slate-700 dark:text-slate-300">Staff:</span>
              <Badge variant="outline" className="font-mono font-bold bg-white dark:bg-slate-900 text-indigo-600 border-indigo-200 dark:border-indigo-800">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
        {filteredEmployees.map((emp) => {
          ...
          const isSelfCard = Boolean(
            currentRole === 'employee' ||
            (currentUser?.employeeId && (currentUser.employeeId === emp.id || currentUser.employeeId === emp.employeeCode)) ||
            (currentUser?.email && emp.email && currentUser.email.toLowerCase() === emp.email.toLowerCase())
          );
          const showSalary = isSalaryVisible(isSelfCard);
          const isChairmanCard =
            (emp as any).role === 'chairman' ||
            emp.designationTitle?.toLowerCase().includes('chairman') ||
            (emp.departmentName?.toLowerCase().includes('board') && !emp.designationTitle?.toLowerCase().includes('assistant'));

          return (
            <Card
              key={emp.id}
              className={`overflow-hidden hover:shadow-lg transition-all duration-200 group flex flex-col justify-between ${
                isChairmanCard
                  ? 'border-purple-500/30 bg-gradient-to-br from-white via-purple-50/20 to-slate-50/40 dark:from-slate-900 dark:via-purple-950/10 dark:to-slate-900'
            ...
            >
              <CardContent className="p-5 sm:p-6 space-y-4">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <img
                      ...
                      alt={emp.firstName}
                      className={`h-12 w-12 rounded-2xl object-cover ring-2 shadow-sm shrink-0 ${
                        isChairmanCard ? 'ring-purple-500/30' : 'ring-indigo-500/20'
                      }`}
                    />
                    <div className="min-w-0">
                      <h3 className="font-bold text-slate-900 dark:text-white text-base leading-tight group-hover:text-indigo-600 transition-colors truncate">
                  ...
                  <span className={`text-[11px] px-2.5 py-0.5 rounded-full font-bold uppercase shrink-0 ${statusBadge.bg} ${statusBadge.text}`}>
                ...
                <div className="space-y-1.5 pt-1">
                  <div className="text-xs font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-1.5 truncate">
                    <Briefcase className={`h-3.5 w-3.5 shrink-0 ${isChairmanCard ? 'text-purple-500' : 'text-indigo-500'}`} />
                    <span className="truncate">{emp.designationTitle}</span>
                  </div>
                ...
                <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs gap-2">
                  <div className="flex flex-col min-w-0">
                    <span className="text-[10px] uppercase font-semibold text-slate-400">
                      {isChairmanCard ? 'Governance Mandate' : 'Current Phase'}
                    </span>
                    <span className={`font-bold capitalize truncate ${isChairmanCard ? 'text-purple-600 dark:text-purple-400' : 'text-indigo-600 dark:text-indigo-400'}`}>
                      {isChairmanCard
                        ? 'Board Governance'
                        : emp.currentLifecycleStage
                        ? String(emp.currentLifecycleStage).replace(/_/g, ' ')
                        : 'Active Service'}
                    </span>
                  </div>
              ...
              <div className="p-3 bg-slate-50/70 dark:bg-slate-800/40 border-t border-slate-100 dark:border-slate-800">
                ...
                >
                  <Link href={`/employees/${emp.id}`}>
                    <span>{isChairmanCard ? 'View Executive Profile & Mandate' : 'View 360° Profile & Timeline'}</span>
